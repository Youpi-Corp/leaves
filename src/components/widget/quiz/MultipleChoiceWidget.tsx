...
import { QuizFeedback, QuizBaseEditor, QuizWidgetViewProps, QuizWidgetEditProps } from '../quiz/QuizWidgetBase'
...
const MultipleChoiceWidgetView: React.FC<QuizWidgetViewProps<MultipleChoiceWidgetProps>> = ({
  widgetData,
  onAnswer,
  readonly = false,
}) => {
  ...
  const displayOptions = (widgetData?.shuffleOptions && options.length > 0)
  ...
  return (
    <div className="w-full">
      <div className="mb-4">
        <h3 className="text-lg font-medium text-gray-900 mb-2">{widgetData.label}</h3>
        ...
        {displayOptions.map((option) => (
          ...
          >
            <div className="flex items-center">
              <div className={`w-5 h-5 flex-shrink-0 mr-2 rounded-${allowMultiple ? 'sm' : 'full'} border ${
                selectedOptions.includes(option.id)
                  ? 'bg-blue-500 border-blue-500 text-white flex items-center justify-center'
                  : 'border-gray-400'
              }`}>
                {selectedOptions.includes(option.id) && <FaCheck className="w-3 h-3" />}
              </div>
      ...
      {showResult && <QuizFeedback isCorrect={isCorrect} feedback={feedback} showFeedback={showFeedback} />}
...
const MultipleChoiceWidgetEdit: React.FC<QuizWidgetEditProps<MultipleChoiceWidgetProps>> = ({
  widgetData,
  onChange,
}) => {
      ...
      <div>
          ...
          <div className="flex space-x-4">
            ...
            <div className="flex items-center">
              <input
                ...
                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
              />
              <label htmlFor="shuffle-options" className="ml-2 text-sm text-gray-700">
                Shuffle options
              </label>
            </div>
          </div>
        ...
        <div className="space-y-2 mb-2">
          {options.map((option, index) => (
            <div key={option.id} className="flex items-start border border-gray-200 p-2 rounded-md">
              <div className="flex flex-col items-center mr-2 space-y-1">
                ...
                <button
                  ...
                  className={`p-1 rounded ${
                    index === options.length - 1 ? 'text-gray-300 cursor-not-allowed' : 'text-gray-500 hover:bg-gray-100'
                  }`}
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
// Register the widget
registerWidget<MultipleChoiceWidgetProps>(multipleChoiceWidgetMetadata, MultipleChoiceWidget);

export { MultipleChoiceWidget, MultipleChoiceWidgetView, MultipleChoiceWidgetEdit };
